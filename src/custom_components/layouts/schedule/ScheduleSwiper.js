...
class ScheduleSwiper extends React.Component {
    ...
    touchDraw = (event) => {
        ...
        let width = document.getElementsByClassName("scheduleSwiper_Page")[0].clientWidth;
        pages[centralId - 1].classList.add("animating");
        pages[centralId].classList.add("animating");
        pages[centralId + 1].classList.add("animating");

        document.querySelector(`.scheduleSwiper_Page:nth-child(${centralId})`)
            .style.transform = `translate(${-width + this.shift}px, 0)`;
        document.querySelector(".scheduleSwiper_Page.center")
            .style.transform = `translate(${this.shift}px, 0)`;
        document.querySelector(`.scheduleSwiper_Page:nth-child(${centralId + 2})`)
            .style.transform = `translate(${width + this.shift}px, 0)`;
    };
